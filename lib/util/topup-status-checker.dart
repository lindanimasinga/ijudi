...
mixin TopTupStatusChecker {
  ...
  CardsOnFile paymentCardselected;
  ...
  void deleteAllLinkedCards() {
    ukhesheService.fetchCardsOnFile(wallet.customerId)
        .asStream()
        .asyncExpand((event) => Stream.fromIterable(event))
        .map((card) => ukhesheService..deleteCardsOnFile(wallet.customerId, card.tokenKey))
        .listen((card) {
          paymentCards = [];
